            ...
            <h4 class="card-title">{{ $title }}
                @can('tagihan-create')
                    <a class="btn btn-success btn-sm float-right"
                        href="{{ route(config('pathadmin.admin_prefix').'tagihans.create') }}"> Tambah Baru</a>
                @endcan
            </h4>
        </div>
                    ...
                    <table class="table table-striped mb-0">
                        ...
                        <tbody>
                            @foreach ($tagihans as $key => $tagihan)
                                <tr>
                                    <td class="text-center">{{ $key + 1 + $valuepage }}</td>
                                    ...
                                    <td class="text-center">
                                        @can('tagihan-edit')
                                            <form style="display:inline"
                                                action="{{ url('/' . config('pathadmin.admin_name') . '/proses/' . $tagihan->id . '/batal-bayar') }}"
                                                method="post">
                                                @csrf
                                                <button type="submit" class="btn btn-primary btn-sm">Batal Bayar</button>
                                            </form>
                                        @endcan

                                        <a href="{{ route(config('pathadmin.admin_prefix').'tagihans.cetak', $tagihan->id) }}"
