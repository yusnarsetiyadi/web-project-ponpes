        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ $title }}
                    @can('orangtuawali-create')
                        <a class="btn btn-success btn-sm float-right" href="{{ route(config('pathadmin.admin_prefix').'orangtuawalis.create',['santri'=> Request::get("santri")]) }}"> Tambah
                            Baru</a>
                    @endcan
                </h4>
            </div>
...
<th>Kontak</th>
                            ...
                            <tbody>
                                @foreach ($orangtuawalis as $key => $orangtuawali)
                                    <tr>
                                        <td>
                                            <center>{{ $key + 1 + $valuepage }}</center>
                                        </td>
                                        <td>{{$orangtuawali->nama}}</td>
<td>{{$orangtuawali->pekerjaan}}</td>
<td>{{$orangtuawali->penghasilan_perbulan}}</td>
<td>{{$orangtuawali->kontak}}</td>

                                        <td>
                                            <center>
                                                @can('orangtuawali-edit')
                    ...
                    {{ $orangtuawalis->appends(['keyword' => Request::get('keyword')])->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
