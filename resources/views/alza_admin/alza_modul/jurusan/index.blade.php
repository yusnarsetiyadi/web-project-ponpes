        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ $title }}
                    @can('jurusan-create')
                        <a class="btn btn-success btn-sm float-right" href="{{ route(config('pathadmin.admin_prefix').'jurusans.create') }}"> Tambah
                            Baru</a>
                    @endcan
                </h4>
            </div>
...
<td>{{$jurusan->aktif}}</td>
                    ...
                    {{ $jurusans->appends(['keyword' => Request::get('keyword')])->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
