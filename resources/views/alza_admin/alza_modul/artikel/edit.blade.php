    ...
    <div class="card">
        ...
        <div class="card-content">
            <div class="card-body">
                ...
                @endif
                {!! Form::model($artikel, ['method' => 'PATCH', 'route' =>
                [config('pathadmin.admin_prefix').'artikels.update', $artikel->id],'enctype' => 'multipart/form-data'])
                !!}
                <div class="form-body">
                    <div class="row">

                        <div class="col-12">
                            <div class="form-group"><label>Kategori</label><select name="kategori_id" class="form-control">
                                <option value="-- PILIH --">-- PILIH --</option>
                                @foreach ($kategori as $row)
                                    <option value="{{ $row->id }}" {!! $artikel->kategori_id == $row->id ? 'selected' : '' !!}> {{ $row->nama }}</option>
                                @endforeach
                            </select></div>
                        </div>
                        ...
                        <div class="col-12 d-flex justify-content-end border-top">
                            ...
                            <a class="btn btn-light-secondary btn-sm mr-1 mb-1 mt-1"
                                href="{{ route(config('pathadmin.admin_prefix').'artikels.index') }}"> Batal</a>
                        </div>
                    </div>
                </div>
                {!! Form::close() !!}
