    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">{{ $title }}
                ...
                <a class="btn btn-success btn-sm float-right"
                    href="{{ route(config('pathadmin.admin_prefix').'events.create') }}"> Tambah
                    Baru</a>
                @endcan
            </h4>
        </div>
        <div class="card-content">
            <div class="card-body">
                                ...
                                <td>
                                    <center>
                                        ...
                                        <a class="btn btn-primary btn-sm"
                                            href="{{ route(config('pathadmin.admin_prefix').'events.edit', $event->id) }}">Ubah</a>
                                        @endcan
                                        @can('event-delete')
                                        <form action="{{ route(config('pathadmin.admin_prefix').'events.destroy', $event->id) }}" method="POST" style="display:inline">
                                            @csrf
                                            @method('DELETE')
                ...
                {{ $events->appends(['keyword' => Request::get('keyword')])->links('pagination::bootstrap-4') }}
            </div>
        </div>
    </div>
</div>
